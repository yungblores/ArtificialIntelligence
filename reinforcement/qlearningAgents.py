...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(self.getLegalActions(state)) == 0:
            ...
            allOptions = [(self.getQValue(state, action), action) for action in self.getLegalActions(state)]
            maxVal = max([option[0] for option in allOptions])
            goodOptions = [option[1] for option in allOptions if option[0] == maxVal]
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.qValues[(state, action)]  = (1 - self.alpha) * currentq + self.alpha * (reward + self.discount * maxq)
