...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {PAC: [], GHOST0: [], GHOST1: [], OBS0: [], OBS1: []}
    ...
    distances = []
    max_manhattan = X_RANGE + Y_RANGE - 2 + MAX_NOISE + 1
    for i in range(max_manhattan):
        distances.append(i)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        evidenceVariablesSet = set(evidenceDict.keys())
        queryVariablesSet = set(queryVariables)
        bigSet = evidenceVariablesSet.union(queryVariablesSet)
        size = len(bigSet)
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        while len(factors) > size:
            eliminationVariable = eliminationOrder.pop(0)
            factors, joinedFactor = joinFactorsByVariable(factors, eliminationVariable)
            if len(joinedFactor.unconditionedVariables()) > 1:
                factors.append(eliminate(joinedFactor, eliminationVariable))
        
        fullJoint = joinFactors(factors)
        normalized = normalize(fullJoint)
        return normalized
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        probabilities = list(self.values())
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for lastPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, lastPos)
            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += self.beliefs[lastPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            p = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            beliefs[particle] += p
    ...
    def elapseTime(self, gameState):
        ...
        """
        particles = []
        for lastPos in self.particles:
            newPosDist = self.getPositionDistribution(gameState, lastPos)
            newPos = newPosDist.sample()
            particles.append(newPos)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = allPositions[:self.numParticles]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for eachParticle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                observationProb = self.getObservationProb(observation[ghostIndex], pacmanPosition, eachParticle[ghostIndex], self.getJailPosition(ghostIndex))
                probability *= observationProb

            beliefs[eachParticle] += probability
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            newParticles = []
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                distance = self.getPositionDistribution(gameState, oldParticle, i, self.ghostAgents[i])
                newParticle[i] = distance.sample()
            particles.append(tuple(newParticle))
        self.particles = particles
