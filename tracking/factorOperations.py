...
def joinFactors(factors: List[Factor]):
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        probabilities = [factor.getProbability(assignment) for factor in factors]
        total_probability = 1
        for probability in probabilities:
            total_probability *= probability
        newFactor.setProbability(assignment, total_probability)
    return newFactor
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        conditionedVariables = factor.conditionedVariables() - {eliminationVariable}
        variableDomainsDict = factor.variableDomainsDict()
        del variableDomainsDict[eliminationVariable]
